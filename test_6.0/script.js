...
let visiblePanels = 1; // Start with only panelOne visible

// Hide panels 2, 3, and 4 initially
document.getElementById('panelTwo').style.display = 'none';
document.getElementById('panelThree').style.display = 'none';
document.getElementById('panelFour').style.display = 'none';

// Add panel button click handler
addButton.addEventListener('click', () => {
    if (visiblePanels < 4) {
        const panelToShow = document.getElementById(panels[visiblePanels]);
        panelToShow.style.display = '';  // Remove inline style to use CSS default
        visiblePanels++;
    }
});

// Remove panel button click handler
removeButton.addEventListener('click', () => {
    if (visiblePanels > 1) {
        visiblePanels--;
        const panelToHide = document.getElementById(panels[visiblePanels]);
